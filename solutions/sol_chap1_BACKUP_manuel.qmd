...
l'exercice [voir Exercice TODO-echantillon:min],
$$
\begin{aligned}
\prob{X_{(1)} > m}
&= 1 - \prob{X_{(1)} \leq m} \\
...
l'exercice [voir Exercice TODO-echantillon:min], on a
que
$$
\begin{aligned}
F_{X_{(1)}}(x)
...
De l'exercice [voir Exercice TODO-echantillon:min], on
a
$$
\begin{aligned}
F_{X_{(1)}}(x)
...
Ici, $f_X(x)=1$ pour $x\in(0,1)$ et $F_X(x)=x$, pour $x\in(0,1)$. Selon l'exercice [voir Exercice TODO-etendue], on trouve que
$$
\begin{aligned}
...
Soit $R$ l'étendue de l'échantillon aléatoire. Avec l'exercice [voir Exercice TODO-etendueunif], on sait que
$$
...
est représentée à la figure [voir TODO-fig:echantillon:domaine]. On a
donc
$$
\begin{aligned}
\prob{X_1 \geq 2X_2 \cup X_2 \geq 2X_1}
...
$$
On remarque que $X_{(n)} \sim \text{Bêta}(n, 1)$. Ainsi, depuis l'exercice [voir Exercice TODO-conjointe], la densité conjointe de $(X_{(1)},X_{(n)})$ peut être exprimée comme suit, pour $0<x<y<1$,
$$
